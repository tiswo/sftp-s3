 ...
 * @since 18/2/8 下午2:38
 */
public class SFTPSubsystem extends SftpSubsystem{
    ...
    protected void doReadDir(Buffer buffer, int id) throws IOException {
        ...
        Buffer reply = null;
        try {
            DirectoryHandle dh = validateHandle(handle, h, RouterDirectoryHandle.class);
            if (dh.isDone()) {
    ...
    protected String doOpenDir(int id, String path, Path p, LinkOption... options) throws IOException {
        ...
        if (!status) {
            ...
            DirectoryHandle dirHandle = new RouterDirectoryHandle(this, p, handle);
    ...
    protected void doOpen(Buffer buffer, int id) throws IOException {
        ...
        String handle;
        try {
            if (access == (SftpConstants.ACE4_WRITE_DATA | SftpConstants.ACE4_WRITE_ATTRIBUTES)) {
                User user = serverSession.getAttribute(Constants.USER_SESSION_KEY);
                if (!user.isWritepermission()) {
                    sendStatus(BufferUtils.clear(buffer), id, SftpConstants.SSH_FX_PERMISSION_DENIED, path);
                    return;
                }
            }
    ...
    protected NavigableMap<String, Object> getAttributes(Path file, int flags, LinkOption... options) throws IOException {
        ...
        resolveWritePermission(attrs);
        return attrs;
    }

    private void resolveWritePermission(NavigableMap<String, Object> attrs){
        User user = serverSession.getAttribute(Constants.USER_SESSION_KEY);
        if (!user.isWritepermission()) {
    ...
    protected AbstractMap.SimpleImmutableEntry<Path, Boolean> validateRealPath(int id, String path, Path f, LinkOption... options) throws IOException {
        return super.validateRealPath(id,path,f,options);
    }
}
